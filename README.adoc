...
collection, so you need add only the following Maven snippet:
...
logic. In Spring Boot applications, if you annotate it with `@Route`, it is automatically
picked up and shown at the root of your web application. You can customize the URL where
the view is shown by giving a parameter to the `@Route` annotation. The following listing
(from the `initial` project at
...
show only the topmost results by employing paging or using lazy loading, for examply using the `grid.setItems(VaadinSpringDataHelpers.fromPagingRepository(repo))` method.
...
define the `ValueChangeMode.LAZY` on the filter text field. The following example shows
